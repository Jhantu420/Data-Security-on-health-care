...
exports.login = async (req, res, next) => {
  try {
    ...
    const token = jwt.sign(
      { userId: user._id, customId: user.customId },
      "your-secret-key",
      {
        expiresIn: "15d",
      }
...
exports.updatethesymptomsTbele = async (req, res, next) => {
  try {
    ...
    if (updateData && updateData.symptoms) {
      // Access the 'symptoms' property
    } else {
      console.error("Symptoms data is missing or undefined");
    }
